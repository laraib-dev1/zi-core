// Full blog listing — same layout pattern as Portfolio page (navbar, banner, grid, footer).
...
export default function BlogPage() {
  ...
  const [blogBanner, setBlogBanner] = useState<BannerType | null>(null);
      ...
      <main className={`${spacing.navbar.offset} ${spacing.navbar.gapBottom} flex-1 w-full`}>
        <Container12 grid gap="gap-6" className={spacing.section.gap}>
          <div className="col-span-12">
            <Banner imageSrc={blogBanner?.imageUrl || "/hero.png"} alt="Blog Banner" />
          </div>

          <div className="col-span-12 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 className="text-2xl font-bold theme-heading m-0 p-0">Blog</h2>
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="relative w-full sm:w-[180px]" ref={dropdownRef}>
                ...
                >
                  <span>{selectedCategory ? selectedCategory : `All (${items.length})`}</span>
                    ...
                    {categoryNames.map((name) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            navigate(`/blogpage?category=${encodeURIComponent(name)}`);
                            setDropdownOpen(false);
                          }}
                          className={`w-full px-3 py-2 text-sm flex items-center justify-between hover:bg-gray-100 transition-colors text-left ${
